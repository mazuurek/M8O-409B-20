...
def analyt_func(x, a, t):
    return np.exp(-a * t) * np.sin(x)

def func_border1(a, t):
    return np.exp(-a * t)

def func_border2(a, t):
    return -np.exp(-a * t)
    ...
    for k in range(K - 1):
        ...
        for j in range(1, N - 1):
            U[k + 1, j] = tau * (a * (U[k, j - 1] - 2 * U[k, j] + U[k, j + 1]) / h**2 +
                                b * (U[k, j + 1] - U[k, j - 1]) / (2 * h) + c * U[k, j]) + U[k, j]
        if approx == 1:
            U[k + 1, 0] = (h * func_border1(a, t) - U[k + 1, 1]) / (h - 1)
            U[k + 1, N - 1] = (h * func_border2(a, t) + U[k + 1, N - 2]) / (h + 1)
        elif approx == 2:
            U[k + 1, 0] = (2 * h * func_border1(a, t) - 4 * U[k + 1, 1] + U[k + 1, 2]) / (2 * h - 3)
            U[k + 1, N - 1] = (2 * h * func_border2(a, t) + 4 * U[k + 1, N - 2] - U[k + 1, N - 3]) / (2 * h + 3)
        elif approx == 3:
            U[k + 1, 0] = (func_border1(a, t) * h * tau * (2 - b * h) - U[k + 1, 1] * (2 * tau) - U[k, 0] * h**2) / \
                          (-2 * tau - h**2 + c * tau * h**2 + h * tau * (2 - b * h))
            U[k + 1, N - 1] = (func_border2(a, t) * h * tau * (2 + b * h) + U[k + 1, N - 2] * (2 * tau) + U[k, N - 1] * h**2) / \
                              (2 * tau + h**2 - c * tau * h**2 + h * tau * (2 + b * h))
...
def implicit(K, t, tau, h, a1, b1, c1, x, approx):
    ...
    for k in range(0, K - 1):
        ...
        if approx == 1:
            b[0] = 1 - 1 / h
            c[0] = 1 / h
            d[0] = func_border1(a1, t)

            a[N - 1] = -1 / h
            b[N - 1] = 1 + 1 / h
            d[N - 1] = func_border2(a1, t)
        elif approx == 2:
            k0 = 1 / (2 * h) / c[1]
            b[0] = (-3 / (2 * h)) + 1 + a[1] * k0
            c[0] = 2 / h + b[1] * k0
            d[0] = func_border1(a1, t) + d[1] * k0

            k1 = -(1 / (h * 2)) / a[N - 2]
            a[N - 1] = (-2 / h) + b[N - 2] * k1
            b[N - 1] = (3 / (h * 2)) + 1 + c[N - 2] * k1
            d[N - 1] = func_border2(a1, t) + d[N - 2] * k1
        elif approx == 3:
            b[0] = 2 * a1**2 / h + h / tau - c1 * h - (2 - b1 * h)
            c[0] = - 2 * a1**2 / h
            d[0] = (h / tau) * U[k - 1][0] - func_border1(a1, t) * (2 - b1 * h)

            a[N - 1] = -2 * a1**2 / h
            b[N - 1] = 2 * a1**2 / h + h / tau - c1 * h + (2 + b1 * h)
            d[N - 1] = (h / tau) * U[k - 1][N - 1] + func_border2(a1, t) * (2 + b1 * h)
...
def main(N, K, time):
    ...
    while True:
        ...
        U_analytic = analyt_func(x, a, T[dt])
